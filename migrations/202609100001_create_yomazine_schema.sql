...
-- Apply this after enabling Supabase Auth Anonymous Sign-ins in the dashboard.
...
create table public.magazines (
  ...
  video_minutes smallint generated always as (
    ceil(video_duration_seconds::numeric / 60)::smallint
  ) stored,
  reading_minutes smallint not null check (reading_minutes in (3, 5, 10)),
  saved_minutes smallint generated always as (
    greatest(0, ceil(video_duration_seconds::numeric / 60)::integer - reading_minutes)::smallint
  ) stored,
  user_goal text not null check (
    char_length(btrim(user_goal)) between 1 and 200
  ),
...
create table public.magazine_versions (
  ...
  illustration_mode text not null default 'not_requested' check (
    illustration_mode in ('ephemeral_generated', 'fallback_layout', 'not_requested')
  ),
  ...
  check (
    ...
    or
    (status = 'succeeded' and article is not null and failure_code is null and completed_at is not null)
    or
    (status = 'failed' and article is null and failure_code is not null and completed_at is not null)
  ),
  check (
    article is null or (
      jsonb_typeof(article) = 'object'
      and article ?& array[
        'magazineTitle', 'lead', 'keyPoints', 'memorableMoment',
        'episode', 'discovery', 'practicalPoints', 'closing'
      ]
...
comment on table public.magazines is
  'One user-owned magazine input and video snapshot. PDFs, images, raw URLs, and media are never stored.';
comment on table public.magazine_versions is
  'Immutable generation versions. article is validated JSONB only for succeeded versions.';

create index magazines_user_created_at_idx
  on public.magazines (user_id, created_at desc);
create index magazines_expires_at_idx
  on public.magazines (expires_at);
create index magazine_versions_magazine_version_idx
  on public.magazine_versions (magazine_id, version_number desc);
create index magazine_versions_incomplete_created_at_idx
  on public.magazine_versions (created_at)
...
create policy "magazines_select_own"
  on public.magazines for select to authenticated
  using ((select auth.uid()) is not null and user_id = (select auth.uid()));

create policy "magazines_insert_own"
  on public.magazines for insert to authenticated
  with check ((select auth.uid()) is not null and user_id = (select auth.uid()));

create policy "magazines_update_own"
  on public.magazines for update to authenticated
  using ((select auth.uid()) is not null and user_id = (select auth.uid()))
  with check ((select auth.uid()) is not null and user_id = (select auth.uid()));

create policy "magazines_delete_own"
  on public.magazines for delete to authenticated
  using ((select auth.uid()) is not null and user_id = (select auth.uid()));

create policy "magazine_versions_select_own"
  on public.magazine_versions for select to authenticated
  using (
    exists (
      select 1
      from public.magazines
      where magazines.id = magazine_versions.magazine_id
        and magazines.user_id = (select auth.uid())
    )
  );

create policy "magazine_versions_insert_own"
  on public.magazine_versions for insert to authenticated
  with check (
    exists (
      select 1
      from public.magazines
...
create policy "magazine_versions_update_own"
  on public.magazine_versions for update to authenticated
  using (
    exists (
      select 1
      from public.magazines
...
create policy "magazine_versions_delete_own"
  on public.magazine_versions for delete to authenticated
  using (
    exists (
      select 1
      from public.magazines
...
-- Allows the current user to implement the "delete all history" action.
...
-- Intended only for pg_cron. It removes incomplete attempts after 24 hours,
-- then removes expired magazines and now-empty abandoned parents.

...
function planSectionBands(sections, font, fontSize, height, pageCapacities, typography) {
  ...
  for (const section of sections) {
    ...
    while (bodyStart < bodyColumns.length) {
      ...
      const columnCount = Math.min(currentBand().capacity - usedColumns, bodyColumns.length - bodyStart);
      const text = bodyColumns
        .slice(bodyStart, bodyStart + columnCount)
        .flat()
        .map((token) => token.value)
        .join("");
      currentBand().placements.push({ type: "body", text, startColumn: usedColumns, columnCount });

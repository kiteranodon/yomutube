...
export const YOMAZINE_SECTION_HEADINGS = Object.freeze([
  ...
  { key: "userGoalAnswer", title: "あなたの知りたいこと", group: "final" },
  { key: "closing", title: "結び", group: "final" },
]);

const SECTION_AFTER_GAP_COLUMNS = 1;
const SECTION_MIN_BODY_COLUMNS = 1;
const ARTICLE_BAND_COUNT = 3;
const ARTICLE_BAND_GAP = 20;
const MIN_CONTINUATION_COLUMNS = 3;
const MIN_CONTINUATION_FILL = 0.4;
const TITLE_MIN_LAST_LINE_SHARE = 0.2;
...
// JavaScript has no Unicode property escape for small kana, so keep the
// Unicode-defined hiragana and katakana classes together here. The katakana
// class includes the Katakana Phonetic Extensions block and half-width forms.
// This class is intentionally independent from punctuation, brackets, and
// Latin positioning because only small kana use the vertical alternate font.
export const SMALL_KANA_CLASSES = Object.freeze({
  hiragana: /^[ぁぃぅぇぉっゃゅょゎゕゖ]$/u,
  katakana: /^[ァィゥェォッャュョヮヵヶㇰ-ㇿｧ-ｯｬ-ｮ]$/u,
});

export function isSmallKana(character) {
  return SMALL_KANA_CLASSES.hiragana.test(character) || SMALL_KANA_CLASSES.katakana.test(character);
}

function hasNativeVerticalAlternate(character, font, smallKanaFont) {
  ...
  return Boolean(horizontalGlyph && verticalGlyph && horizontalGlyph.id !== verticalGlyph.id);
}
...
function drawVerticalGlyph(page, character, options) {
  const { font, smallKanaFont = font, fontSize, x, y, color = INK } = options;
  ...
  if (verticalPunctuationGlyph) {
    ...
    // This second embedding of the same face enables its native vert/vrt2
    // alternate. The alternate's ink is already placed in the physical
    // upper-right of the em body, so no manual translation is needed. A
    // proportional fallback covers half-width small kana, for which this font
    // has no vertical alternate.
    const usesNativeAlternate = hasNativeVerticalAlternate(character, font, smallKanaFont);
    page.drawText(character, {
      x: x - fontSize * 0.5 + (usesNativeAlternate ? 0 : fontSize * 0.16),
      y: y + (usesNativeAlternate ? 0 : fontSize * 0.15),
      font: usesNativeAlternate ? smallKanaFont : font,
      size: fontSize,
      color,
    });
  } else {
...
function meaningfulTokens(tokens) {
  return tokens.filter((token) => token.type !== "space" && token.type !== "break");
}

function tokenTextLength(tokens) {
  return meaningfulTokens(tokens).reduce((length, token) => length + Array.from(token.value).length, 0);
}

function boundaryPenalty(columns, columnIndex) {
  const before = columns.slice(0, columnIndex).flat();
  const after = columns.slice(columnIndex).flat();
  const previous = meaningfulTokens(before).at(-1);
  const next = meaningfulTokens(after)[0];
  const rawBefore = before.map((token) => token.value).join("");

  if (!previous || !next) return 0;
  if (previous.type === "glyph" && openingBrackets.has(previous.value)) return 1000;
  if (next.type === "glyph" && prohibitedAtColumnStart.has(next.value)) return 1000;
  if (before.at(-1)?.type === "break") return 0;
  if (/[。！？!?][」』）】〉》〕]?$/u.test(rawBefore)) return 0.1;
  if (/[、，,：:；;][」』）】〉》〕]?$/u.test(rawBefore)) return 0.8;
  return 3;
}

function minimumContinuationColumns(capacity) {
  // The requirement is three body columns OR 40% of the available band.
  return Math.max(1, Math.min(MIN_CONTINUATION_COLUMNS, Math.ceil(capacity * MIN_CONTINUATION_FILL)));
}

function balancedColumnChunks(columns, capacities) {
  if (columns.length === 0) return [];
  let bandCount = 1;
  let totalCapacity = capacities[0];
  while (totalCapacity < columns.length && bandCount < capacities.length) {
    totalCapacity += capacities[bandCount];
    bandCount += 1;
  }
  if (totalCapacity < columns.length) return null;
  if (bandCount === 1) return [{ start: 0, end: columns.length, capacity: capacities[0] }];

  const selectedCapacities = capacities.slice(0, bandCount);
  const idealFill = columns.length / selectedCapacities.reduce((sum, capacity) => sum + capacity, 0);
  const memo = new Map();

  const search = (bandIndex, usedColumns) => {
    const key = `${bandIndex}:${usedColumns}`;
    if (memo.has(key)) return memo.get(key);
    const capacity = selectedCapacities[bandIndex];
    const remainingColumns = columns.length - usedColumns;
    if (bandIndex === bandCount - 1) {
      const minimum = minimumContinuationColumns(capacity);
      if (remainingColumns < minimum || remainingColumns > capacity) return null;
      const fill = remainingColumns / capacity;
      return { score: (fill - idealFill) ** 2 * 100, counts: [remainingColumns] };
    }

    const remainingCapacities = selectedCapacities.slice(bandIndex + 1);
    const maximumAfter = remainingCapacities.reduce((sum, value) => sum + value, 0);
    const minimumAfter = remainingCapacities.reduce((sum, value) => sum + minimumContinuationColumns(value), 0);
    const minimumHere = bandIndex === 0 ? 1 : minimumContinuationColumns(capacity);
    let best = null;

    for (let count = minimumHere; count <= Math.min(capacity, remainingColumns); count += 1) {
      const after = remainingColumns - count;
      if (after < minimumAfter || after > maximumAfter) continue;
      const tail = search(bandIndex + 1, usedColumns + count);
      if (!tail) continue;
      const fill = count / capacity;
      const semanticPenalty = boundaryPenalty(columns, usedColumns + count);
      const score = tail.score + (fill - idealFill) ** 2 * 100 + semanticPenalty * 2;
      if (!best || score < best.score) best = { score, counts: [count, ...tail.counts] };
    }
    memo.set(key, best);
    return best;
  };
  ...
  return result.counts.map((count, index) => {
    const chunk = { start, end: start + count, capacity: selectedCapacities[index] };
    start += count;
    return chunk;
  });
}

function titleBoundaryPenalty(tokens, index) {
  const previous = meaningfulTokens(tokens.slice(0, index)).at(-1);
  const next = meaningfulTokens(tokens.slice(index))[0];
  const rawBefore = tokens.slice(0, index).map((token) => token.value).join("");
  if (!previous || !next) return 1000;
  if (previous.type === "glyph" && openingBrackets.has(previous.value)) return 1000;
  if (next.type === "glyph" && prohibitedAtColumnStart.has(next.value)) return 1000;
  if (tokens[index - 1]?.type === "break") return 0;
  if (/[。！？!?][」』）】〉》〕]?$/u.test(rawBefore)) return 0.1;
  if (/[^\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}ー]$/u.test(rawBefore)) return 0.45;
  return 1.5;
}

function titlePartition(tokens, lineCount, font, latinFont, fontSize, height) {
  const totalLength = tokenTextLength(tokens);
  const prefixAdvance = [0];
  for (const token of tokens) prefixAdvance.push(prefixAdvance.at(-1) + tokenAdvance(token, font, fontSize, latinFont));
  const advanceBetween = (start, end) => prefixAdvance[end] - prefixAdvance[start];
  let best = null;

  const visit = (boundaries, start) => {
    if (boundaries.length === lineCount - 1) {
      const points = [0, ...boundaries, tokens.length];
      const lines = [];
      let semanticPenalty = 0;
      for (let lineIndex = 0; lineIndex < lineCount; lineIndex += 1) {
        const from = points[lineIndex];
        const to = points[lineIndex + 1];
        const lineTokens = tokens.slice(from, to);
        const length = tokenTextLength(lineTokens);
        const advance = advanceBetween(from, to);
        if (!length || advance > height) return;
        if (lineIndex > 0 && (length === 1 || length / totalLength < TITLE_MIN_LAST_LINE_SHARE)) return;
        if (lineIndex < lineCount - 1) semanticPenalty += titleBoundaryPenalty(tokens, to);
        lines.push({ tokens: lineTokens, length, advance });
      }
      const mean = lines.reduce((sum, line) => sum + line.advance, 0) / lineCount;
      const imbalance = lines.reduce((sum, line) => sum + ((line.advance - mean) / height) ** 2, 0);
      const score = imbalance * 100 + semanticPenalty;
      if (!best || score < best.score) best = { score, lines };
      return;
    }

    const remainingLines = lineCount - boundaries.length - 1;
    for (let index = start + 1; index <= tokens.length - remainingLines; index += 1) {
      if (advanceBetween(start, index) > height) break;
      visit([...boundaries, index], index);
    }
  };
...
export function planBalancedVerticalTitle(text, options) {
  const { font, latinFont = font, maximum, minimum, width, height, columnGap = 3, numericYearStyle = "original" } = options;
  const attempt = (fontSize, lineCount) => {
    const availableColumns = Math.floor((width + columnGap) / (fontSize + columnGap));
    if (lineCount > availableColumns) return null;
    const tokens = preparedVerticalTokens(text, font, latinFont, fontSize, height, { numericYearStyle, latinFont });
    const partition = titlePartition(tokens, lineCount, font, latinFont, fontSize, height);
    if (!partition) return null;
    return {
      fontSize,
      lines: partition.lines.map((line) => line.tokens.map((token) => token.value).join("")),
      lineLengths: partition.lines.map((line) => line.length),
      lineAdvances: partition.lines.map((line) => line.advance),
    };
  };

  for (const lineCount of [1, 2, 3]) {
    const result = attempt(maximum, lineCount);
    if (result) return result;
  }
  for (let fontSize = maximum - 0.5; fontSize >= Math.max(minimum, maximum - 2); fontSize -= 0.5) {
    const result = attempt(fontSize, 2);
    if (result) return result;
  }
  for (let fontSize = maximum - 0.5; fontSize >= minimum; fontSize -= 0.5) {
    for (const lineCount of [1, 2, 3]) {
      const result = attempt(fontSize, lineCount);
      if (result) return result;
    }
  }
  return null;
...
export function createArticleSections(article) {
  ...
  return YOMAZINE_SECTION_HEADINGS.flatMap((definition) => {
    const value = article[definition.key];
    if (definition.key === "userGoalAnswer" && !cleanText(value)) return [];
...
function articleBandBounds(top, bottom) {
  const bandHeight = (top - bottom - ARTICLE_BAND_GAP * (ARTICLE_BAND_COUNT - 1)) / ARTICLE_BAND_COUNT;
  return Array.from({ length: ARTICLE_BAND_COUNT }, (_, index) => {
    const bandTop = top - index * (bandHeight + ARTICLE_BAND_GAP);
    return { top: bandTop, bottom: bandTop - bandHeight };
  });
}

function planSectionBands(sections, font, fontSize, height, pageCapacities, typography) {
  const pages = [];
  const headingFontSize = fontSize + 2;
  let absoluteBandIndex = -1;
  let usedColumns = 0;
  ...
  const startBand = () => {
    absoluteBandIndex += 1;
    usedColumns = 0;
    const pageIndex = Math.floor(absoluteBandIndex / ARTICLE_BAND_COUNT);
    const bandIndex = absoluteBandIndex % ARTICLE_BAND_COUNT;
    if (!pages[pageIndex]) pages[pageIndex] = { bands: [] };
    pages[pageIndex].bands[bandIndex] = {
      capacity: capacityForPage(pageIndex),
      placements: [],
    };
  };
  const currentBand = () => pages[Math.floor(absoluteBandIndex / ARTICLE_BAND_COUNT)].bands[absoluteBandIndex % ARTICLE_BAND_COUNT];
  const capacityAtBand = (bandIndex) => capacityForPage(Math.floor(bandIndex / ARTICLE_BAND_COUNT));
  ...
  for (const section of sections) {
    ...
    if (section.title) {
      // A subhead always begins a fresh horizontal band at its right edge.
      // Reserving its column, a blank column, and the first body column keeps
      // the heading from becoming an orphan at a band or page boundary.
      if (usedColumns > 0 || currentBand().placements.length > 0) startBand();
      const requiredColumns = 1 + SECTION_AFTER_GAP_COLUMNS + SECTION_MIN_BODY_COLUMNS;
      if (currentBand().capacity < requiredColumns) return null;
      currentBand().placements.push({ type: "heading", title: section.title, startColumn: 0, columnCount: 1 });
      usedColumns += 1 + SECTION_AFTER_GAP_COLUMNS;
    }

    if (usedColumns >= currentBand().capacity) startBand();
    const capacities = [currentBand().capacity - usedColumns];
    let capacityBandIndex = absoluteBandIndex + 1;
    while (capacities.reduce((sum, value) => sum + value, 0) < bodyColumns.length) {
      capacities.push(capacityAtBand(capacityBandIndex));
      capacityBandIndex += 1;
    }
    const chunks = balancedColumnChunks(bodyColumns, capacities);
    if (!chunks) return null;

    for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex += 1) {
      if (chunkIndex > 0) startBand();
      const chunk = chunks[chunkIndex];
      const columnCount = chunk.end - chunk.start;
      const text = bodyColumns
        .slice(chunk.start, chunk.end)
        .flat()
        .map((token) => token.value)
        .join("");
      currentBand().placements.push({
        type: "body",
        text,
        ...
        fillRatio: columnCount / chunk.capacity,
      });
...
function planArticlePages(article, font, typography) {
  const textHeight = articleBandBounds(700, 150)[0].top - articleBandBounds(700, 150)[0].bottom;
  ...
  for (const fontSize of [11.5, 11, 10.5, 10, 9.5, 9, 8.5, 8]) {
    ...
    const introductionPages = planSectionBands(
    ...
    const episodePages = planSectionBands(
      [{ key: "episode", title: null, body: article.episode, group: "episode" }],
      font,
      fontSize,
      textHeight,
      [Math.floor((454 - 251) / (fontSize + 4)), fullPageColumns],
      typography,
    );
    const finalPages = planSectionBands(finalSections, font, fontSize, textHeight, [fullPageColumns], typography);
    if (!introductionPages || !episodePages || !finalPages) continue;

    const pageCount = 3 + introductionPages.length + episodePages.length + finalPages.length;
    if (pageCount <= MAX_PAGES) {
      return { fontSize, introductionPages, episodePages, finalPages, pageCount };
    }
  }
...
function drawThreeBandFlow(page, pagePlan, options) {
  const { font, smallKanaFont = font, latinFont = font, fontSize, right, left, top, bottom, columnGap = 4, numericYearStyle = "original" } = options;
  const columnAdvance = fontSize + columnGap;
  const bands = articleBandBounds(top, bottom);
  let fits = true;

  for (let index = 0; index < bands.length - 1; index += 1) {
    const separatorY = (bands[index].bottom + bands[index + 1].top) / 2;
    page.drawRectangle({ x: left, y: separatorY, width: right - left, height: 0.7, color: MUTED, opacity: 0.2 });
  }

  for (let bandIndex = 0; bandIndex < ARTICLE_BAND_COUNT; bandIndex += 1) {
    const bandPlan = pagePlan.bands[bandIndex];
    if (!bandPlan) continue;
    const band = bands[bandIndex];
    for (const placement of bandPlan.placements) {
      if (placement.type === "heading") {
        ...
        page.drawRectangle({
          ...
          y: band.top - Math.min(54, (band.top - band.bottom) * 0.34),
          width: 2.5,
          height: Math.min(54, (band.top - band.bottom) * 0.34),
          color: ACCENT,
        });
...
export function drawVerticalText(page, value, options) {
  const text = cleanText(value);
  const { font, smallKanaFont = font, latinFont = font, fontSize, right, top, bottom, left = MARGIN, columnGap = 3, color = INK, maxColumns, numericYearStyle = "original", debugCellGuides = false } = options;
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (debugCellGuides && token.type !== "break" && token.type !== "space") {
      page.drawRectangle({
        x: x - fontSize / 2,
        y: y - advance,
        width: fontSize,
        height: advance,
        borderColor: isSmallKana(token.value) ? rgb(0.82, 0.2, 0.18) : rgb(0.2, 0.42, 0.78),
        borderWidth: 0.35,
        opacity: 0.6,
      });
    }
    if (token.type === "glyph") {
      drawVerticalGlyph(page, token.value, { font, smallKanaFont, fontSize, x, y: y - fontSize, color });
...
function fallbackArticle(article) {
  return {
    ...
    userGoalAnswer: article?.userGoalAnswer || "",
...
function fitBalancedVerticalTitle(page, text, options) {
  const { font, latinFont = font, right, left, top, bottom, maximum = 18, minimum = 9, columnGap = 4, numericYearStyle = "original" } = options;
  const plan = planBalancedVerticalTitle(text, {
    font,
    latinFont,
    maximum,
    minimum,
  ...
  if (!plan) return null;
  const columnAdvance = plan.fontSize + columnGap;
  for (let index = 0; index < plan.lines.length; index += 1) {
    const lineRight = right - index * columnAdvance;
    const result = drawVerticalText(page, plan.lines[index], {
      ...options,
      fontSize: plan.fontSize,
      right: lineRight,
      left: lineRight - plan.fontSize,
      maxColumns: 1,
      columnGap: 0,
    });
    if (!result.fits || result.columns !== 1) return null;
  }
  return plan;
}

function drawPageHeading(page, title, font, typography = {}) {
  ...
  drawVerticalText(page, title, { ...typography, font, fontSize: 17, right: 525, top: 712, bottom: 245, left: 492, columnGap: 7 });
}
...
function drawFallbackArtwork(page, box, serifFont, typography = {}) {
  ...
  drawVerticalText(page, "読むための余白", { ...typography, font: serifFont, fontSize: 13, right: box.x + box.width * 0.22, top: box.y + box.height * 0.84, bottom: box.y + box.height * 0.32, left: box.x + 10, color: INK });
}
...
export async function generateYomazinePdf(input) {
  ...
  const smallKanaFont = await pdf.embedFont(serifBytes, {
    // pdf-lib/fontkit can omit GSUB-only alternate glyphs when this embedding
    // is subsetted, so retain the full face to keep every small-kana alternate.
    subset: false,
    features: { vert: true, vrt2: true },
  });
  const typography = { numericYearStyle, latinFont, smallKanaFont };
  ...
  if (coverImage) drawImageContain(page, coverImage, coverBox); else drawFallbackArtwork(page, coverBox, serifFont, typography);
  ...
  const coverTitlePlan = fitBalancedVerticalTitle(page, article.magazineTitle, { ...typography, font: serifFont, right: 484, left: 414, top: 660, bottom: 205, maximum: 21, minimum: 9, columnGap: 8 });
  if (coverTitlePlan) titlePlans.push({ location: "cover", ...coverTitlePlan });
  contentFits = Boolean(coverTitlePlan) && contentFits;
  ...
  drawPageHeading(page, "目次", serifFont, typography);
  drawPanel(page, { x: 279, y: 118, width: 179, height: 618 }, rgb(1, 1, 1));
  drawVertical(page, "この一冊の流れ", { font: serifFont, fontSize: 14, right: 435, top: 700, bottom: 470, left: 406, color: ACCENT_DARK });
  const introductionPage = 3;
  const episodePage = introductionPage + articlePlan.introductionPages.length;
  const finalSectionPage = episodePage + articlePlan.episodePages.length;
  const sourcePage = articlePlan.pageCount;
  const tocItems = [
    `${japaneseNumber(introductionPage)}頁　導入`,
    `${japaneseNumber(episodePage)}頁　具体的なエピソード`,
    `${japaneseNumber(finalSectionPage)}頁　発見・実践・知りたいこと・結び`,
  ...
  drawVertical(page, "動画情報", { font: serifFont, fontSize: 15, right: 232, top: 700, bottom: 505, left: 200, color: ACCENT_DARK });
  ...
  articlePlan.introductionPages.forEach((sectionPage, index) => {
    ...
    drawPageHeading(page, index === 0 ? "導入" : "導入・続き", serifFont, typography);
    if (index === 0) {
      drawVertical(page, "記事タイトル", { font: serifFont, fontSize: 8.5, right: 461, top: 710, bottom: 555, left: 443, color: ACCENT_DARK });
      const articleTitlePlan = fitBalancedVerticalTitle(page, article.magazineTitle, { ...typography, font: serifFont, right: 430, left: 367, top: 710, bottom: 150, maximum: 16, minimum: 9, columnGap: 7 });
      if (articleTitlePlan) titlePlans.push({ location: "article", ...articleTitlePlan });
      contentFits = Boolean(articleTitlePlan) && contentFits;
      drawPanel(page, { x: 52, y: 118, width: 288, height: 618 }, rgb(1, 1, 1));
      contentFits = drawThreeBandFlow(page, sectionPage, { ...typography, font: serifFont, fontSize: articlePlan.fontSize, right: 316, left: 73, top: 700, bottom: 150, columnGap: 4 }) && contentFits;
    } else {
      drawPanel(page, { x: 52, y: 118, width: 406, height: 618 }, rgb(1, 1, 1));
      contentFits = drawThreeBandFlow(page, sectionPage, { ...typography, font: serifFont, fontSize: articlePlan.fontSize, right: 432, left: 74, top: 700, bottom: 150, columnGap: 4 }) && contentFits;
    }
    pageNumber += 1;
  });

  articlePlan.episodePages.forEach((episodePagePlan, index) => {
    page = addPageBase(pdf, pageNumber, dateLabel, latinFont);
    drawPageHeading(page, index === 0 ? "具体的なエピソード" : "具体的なエピソード・続き", serifFont, typography);
    if (index === 0) {
      drawPanel(page, { x: 52, y: 118, width: 180, height: 618 });
      const episodeBox = { x: 64, y: 410, width: 156, height: 294 };
      if (articleImage) drawImageContain(page, articleImage, episodeBox); else drawFallbackArtwork(page, episodeBox, serifFont, typography);
      drawVertical(page, "場面のリズムを読む", { font: serifFont, fontSize: 13, right: 198, top: 368, bottom: 165, left: 170, color: ACCENT_DARK });
      page.drawCircle({ x: 91, y: 196, size: 24, color: ACCENT });
      page.drawCircle({ x: 128, y: 158, size: 9, color: rgb(1, 1, 1) });
      contentFits = drawThreeBandFlow(page, episodePagePlan, { ...typography, font: serifFont, fontSize: articlePlan.fontSize, right: 454, left: 251, top: 700, bottom: 150, columnGap: 4 }) && contentFits;
    } else {
      drawPanel(page, { x: 52, y: 118, width: 406, height: 618 }, rgb(1, 1, 1));
      contentFits = drawThreeBandFlow(page, episodePagePlan, { ...typography, font: serifFont, fontSize: articlePlan.fontSize, right: 432, left: 74, top: 700, bottom: 150, columnGap: 4 }) && contentFits;
    }
    pageNumber += 1;
  });
  ...
  articlePlan.finalPages.forEach((sectionPage, index) => {
    page = addPageBase(pdf, pageNumber, dateLabel, latinFont);
    drawPageHeading(page, index === 0 ? "発見・実践・回答・結び" : "発見・実践・回答・結び・続き", serifFont, typography);
    drawPanel(page, { x: 52, y: 118, width: 406, height: 618 }, rgb(1, 1, 1));
    contentFits = drawThreeBandFlow(page, sectionPage, { ...typography, font: serifFont, fontSize: articlePlan.fontSize, right: 432, left: 74, top: 700, bottom: 150, columnGap: 4 }) && contentFits;
    pageNumber += 1;
  });
  ...
  page = addPageBase(pdf, pageNumber, dateLabel, latinFont);
  drawPageHeading(page, "読書ログ・出典", serifFont, typography);
  drawPanel(page, { x: 306, y: 118, width: 152, height: 618 }, rgb(1, 1, 1));
  drawVertical(page, "読書ログ", { font: serifFont, fontSize: 15, right: 432, top: 700, bottom: 520, left: 402, color: ACCENT_DARK });
  ...
  drawVertical(page, "出典情報", { font: serifFont, fontSize: 15, right: 259, top: 700, bottom: 520, left: 228, color: ACCENT_DARK });
  ...
  drawVertical(page, "正規化URL", { font: serifFont, fontSize: 8, right: 80, top: 690, bottom: 575, left: 62, color: ACCENT_DARK });
  ...
  const bodyBands = [
    ["introduction", articlePlan.introductionPages],
    ["episode", articlePlan.episodePages],
    ["final", articlePlan.finalPages],
  ].flatMap(([group, plannedPages]) => plannedPages.flatMap((plannedPage, plannedPageIndex) => (
    plannedPage.bands.flatMap((band, bandIndex) => (
      (band?.placements || [])
        .filter((placement) => placement.type === "body")
        .map((placement) => ({
          group,
          pageIndex: plannedPageIndex,
          bandIndex,
          flowId: placement.flowId,
          chunkIndex: placement.chunkIndex,
          chunkCount: placement.chunkCount,
          continuation: placement.continuation,
          columns: placement.columnCount,
  ...
  return {
    bytes,
    filename: `yomazine-${iso}.pdf`,
    layoutDiagnostics: {
      titlePlans,
      bodyBands,
      continuationBands: bodyBands.filter((band) => band.continuation),
    },
  };
}

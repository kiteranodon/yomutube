...
function verticalColumnCount(text, font, fontSize, height) {
  const columns = verticalTokenColumns(text, font, fontSize, height);
  return columns ? columns.length : Number.POSITIVE_INFINITY;
}

function verticalTokenColumns(text, font, fontSize, height) {
...
function splitVerticalText(value, font, fontSize, height, firstPageColumns, continuationColumns) {
  const columns = verticalTokenColumns(value, font, fontSize, height);
...
function planArticlePages(article, font) {
  ...
  for (const fontSize of [11.5, 11, 10.5, 10, 9.5, 9, 8.5, 8]) {
    ...
    const introductionChunks = splitVerticalText(
      introduction,
      font,
      fontSize,
      textHeight,
      Math.floor((316 - 73) / (fontSize + 5)),
      fullPageColumns,
    );
    ...
    const finalChunks = splitVerticalText(finalSection, font, fontSize, textHeight, fullPageColumns, fullPageColumns);
    if (!introductionChunks || !episodeChunks || !finalChunks) continue;

    const pageCount = 3 + introductionChunks.length + episodeChunks.length + finalChunks.length;
    if (pageCount <= MAX_PAGES) {
      return { fontSize, introductionChunks, episodeChunks, finalChunks, pageCount };
    }
  }
...
export async function generateYomazinePdf(input) {
  ...
  const articlePlan = planArticlePages(article, serifFont);
  if (!articlePlan) {
  ...
  const episodePage = introductionPage + articlePlan.introductionChunks.length;
  ...
  let pageNumber = 3;
  articlePlan.introductionChunks.forEach((chunk, index) => {
    page = addPageBase(pdf, pageNumber, dateLabel, serifFont);
    ...
    if (index === 0) {
      ...
      contentFits = drawVerticalText(page, chunk, { font: serifFont, fontSize: articlePlan.fontSize, right: 316, left: 73, top: 700, bottom: 150, columnGap: 5 }).fits && contentFits;
    } else {
      drawPanel(page, { x: 52, y: 118, width: 406, height: 618 }, rgb(1, 1, 1));
      contentFits = drawVerticalText(page, chunk, { font: serifFont, fontSize: articlePlan.fontSize, right: 432, left: 74, top: 700, bottom: 150, columnGap: 4 }).fits && contentFits;
    }
    pageNumber += 1;
  });
  ...
  articlePlan.episodeChunks.forEach((chunk, index) => {
    ...
    if (index === 0) {
      ...
      contentFits = drawVerticalText(page, chunk, { font: serifFont, fontSize: articlePlan.fontSize, right: 454, left: 251, top: 700, bottom: 150, columnGap: 4 }).fits && contentFits;
    } else {
      drawPanel(page, { x: 52, y: 118, width: 406, height: 618 }, rgb(1, 1, 1));
      contentFits = drawVerticalText(page, chunk, { font: serifFont, fontSize: articlePlan.fontSize, right: 432, left: 74, top: 700, bottom: 150, columnGap: 4 }).fits && contentFits;
    }
  ...
  articlePlan.finalChunks.forEach((chunk, index) => {
    page = addPageBase(pdf, pageNumber, dateLabel, serifFont);

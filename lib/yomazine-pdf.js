...
function preparedVerticalTokens(value, font, latinFont, fontSize, columnHeight, typography) {
  const maxTextLength = Math.max(fontSize, columnHeight - fontSize * 1.18);
...
function tokenAdvance(token, font, fontSize, latinFont = font) {
  ...
  if (token.type === "rotated" || token.type === "short-latin") {
    ...
    return mixedTextWidth(token.value, font, latinFont, fontSize * 0.78) + lineAdvance;
  }
  return lineAdvance;
}
...
function planArticlePages(article, font, typography) {
  ...
  for (const fontSize of [11.5, 11, 10.5, 10, 9.5, 9, 8.5, 8]) {
    ...
    const finalChunks = splitVerticalText(finalSection, font, fontSize, textHeight, fullPageColumns, fullPageColumns, typography);
...
export function drawVerticalText(page, value, options) {
  ...
  const tokens = preparedVerticalTokens(text, font, latinFont, fontSize, Math.max(fontSize, usableHeight - fontSize), typography);
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (shouldWrapBeforeToken(tokens, index, remaining, usableHeight - fontSize, font, fontSize, latinFont)) {
      x -= columnAdvance;
      y = top - fontSize;
      columns += 1;
    }
    if (token.type === "glyph") {
      ...
      drawHorizontalRun(page, token.value, { x: x - width / 2, y: y + fontSize * 0.08, font, latinFont, size: tcySize, color });
...
function drawRotatedLatinLine(page, text, options) {
  ...
  drawClockwiseRun(page, value, { x: x - fittedSize * 0.38, y: top, font, latinFont, size: fittedSize, color });
  return true;
}
...
export async function generateYomazinePdf(input) {
  ...
  articlePlan.introductionChunks.forEach((chunk, index) => {
    ...
    if (index === 0) {
      ...
      contentFits = drawVertical(page, chunk, { font: serifFont, fontSize: articlePlan.fontSize, right: 316, left: 73, top: 700, bottom: 150, columnGap: 5 }).fits && contentFits;
    } else {

...
function tokenAdvance(token, font, fontSize) {
  ...
  if (token.type === "latin") return font.widthOfTextAtSize(token.value, fontSize * 0.78) + fontSize * 0.5;
  return lineAdvance;
}
...
function verticalColumnCount(text, font, fontSize, height) {
  ...
  for (const token of verticalTokens(text)) {
    const advance = tokenAdvance(token, font, fontSize);
    if (advance > remaining && remaining < columnHeight) {
      columns += 1;
...
export function drawVerticalText(page, value, options) {
  ...
  for (const token of verticalTokens(text)) {
    const advance = tokenAdvance(token, font, fontSize);
    if (y - advance < bottom && y < top - fontSize) {
      x -= columnAdvance;
      y = top - fontSize;
      columns += 1;
    }
    if (token.type === "glyph") {
      ...
      // Rotate each Latin word/run clockwise so it reads from the top of the
      // current vertical column toward the bottom. This preserves column flow
      // without scattering individual letters across the page.
